...
void main() {
	...
	if (mat != 50004) {
		...
		if (emissive && length(lightcol) < 0.001) {
			vec4[10] lightcols0;
			vec4 lightcol0 = texture2D(gtexture, texCoord) * vec4(vertexCol.rgb, 1);
			...
			for (int i = 0; i < 9; i++) {
				lightcols0[i] = texture2D(gtexture, texCoord + offsets[i] * spriteSize / atlasSize) * vec4(vertexCol.rgb, 1);
